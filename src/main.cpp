...
#if defined(NDEBUG)
# error "Everestcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8a74dfeebd82360030d5d48831339340a82a750b872b57728e80b4cd105d879f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Everestcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Everestcoin Signed Message:\n";
...
{
    // Everestcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Everestcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Everestcoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Everestcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Everestcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Everestcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8a74dfeebd82360030d5d48831339340a82a750b872b57728e80b4cd105d879f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Everestcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1475435423;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1475754;
        ...
        assert(block.hashMerkleRoot == uint256("0x05222d4f0cda852da164c06803ab406110e2802df47ba6ab59fa60c8f2254141"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Everestcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EverestcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EverestcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EverestcoinMiner(CWallet *pwallet)
{
    printf("EverestcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("everestcoin-miner");
    ...
    try { loop {
        ...
        printf("Running EverestcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EverestcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EverestcoinMiner, pwallet));
}
